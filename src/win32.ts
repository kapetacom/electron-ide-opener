...
type WindowsExternalEditorPathInfo =
    | {
           ...
           **/
          readonly installLocationRegistryKey?: 'InstallLocation' | 'UninstallString';
...
const registryKeysForJetBrainsIDE = (product: string): ReadonlyArray<RegistryKey> => {
...
const executableShimPathsForJetBrainsIDE = (baseName: string): ReadonlyArray<ReadonlyArray<string>> => {
    return [
        ['bin', `${baseName}64.exe`],
        ['bin', `${baseName}.exe`],
    ];
...
const validateStartsWith = (registryVal: string, definedVal: string[]): boolean => {
...
const editors: WindowsExternalEditor[] = [
    ...
    {
        name: 'Brackets',
        registryKeys: [Wow64LocalMachineUninstallKey('{4F3B6E8C-401B-4EDE-A423-6481C239D6FF}')],
    ...
    },
    {
        ...
        registryKeys: [Wow64LocalMachineUninstallKey('{2D6C1116-78C6-469C-9923-3E549218773F}')],
...
function getKeyOrEmpty(keys: ReadonlyArray<RegistryValue>, key: string): string {
    const entry = keys.find((k) => k.name === key);
    return entry && entry.type === RegistryValueType.REG_SZ ? entry.data : '';
}

function getAppInfo(editor: WindowsExternalEditor, keys: ReadonlyArray<RegistryValue>): IWindowsAppInformation {
...
async function findApplication(editor: WindowsExternalEditor) {
    for (const { key, subKey } of editor.registryKeys) {
        ...
        if (!validateStartsWith(displayName, editor.displayNamePrefixes) || !editor.publishers.includes(publisher)) {
            console.debug(`Unexpected registry entries for ${editor.name}`);
            continue;
        }
        ...
        for (const path of executableShimPaths) {
            ...
            console.debug(`Executable for ${editor.name} not found at '${path}'`);
        }
    }
...
async function findJetBrainsToolboxApplication(editor: WindowsExternalEditor) {
    ...
    const toolboxRegistryReference = [CurrentUserUninstallKey('toolbox'), Wow64LocalMachineUninstallKey('toolbox')];
